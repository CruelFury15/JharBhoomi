    ...
    <article className="overflow-hidden rounded-2xl border border-yellow-200 bg-white shadow-sm transition hover:shadow-md dark:border-yellow-800 dark:bg-yellow-900">
      ...
      <div className="space-y-4 p-5">
        <div className="space-y-1">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-yellow-50">{props.cardtitle}</h3>
          <p className="text-sm text-slate-700 dark:text-yellow-300">{props.cardtext}</p>
        </div>

        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-700 dark:text-yellow-200">
        ...
        {(props.l1 || props.l2 || props.l3) && (
          <ul className="grid grid-cols-1 gap-2 text-sm text-slate-700 dark:text-yellow-200">
            {props.l1 ? <li className="rounded-lg bg-slate-50 px-3 py-2 ring-1 ring-slate-200 dark:bg-yellow-950 dark:ring-yellow-800">{props.l1}</li> : null}
            {props.l2 ? <li className="rounded-lg bg-slate-50 px-3 py-2 ring-1 ring-slate-200 dark:bg-yellow-950 dark:ring-yellow-800">{props.l2}</li> : null}
            {props.l3 ? <li className="rounded-lg bg-slate-50 px-3 py-2 ring-1 ring-slate-200 dark:bg-yellow-950 dark:ring-yellow-800">{props.l3}</li> : null}
          </ul>
        )}
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-yellow-200 bg-white text-slate-700 shadow-sm hover:bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-900 dark:text-yellow-200 dark:hover:bg-yellow-800"
            aria-label="Share"
          >
            <img src={share} alt="share" className="h-5 w-5" />
          </button>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-yellow-200 px-5 py-4 text-sm dark:border-yellow-800">
        <span className="text-slate-600 dark:text-yellow-300">Starting From</span>
        <span className="text-base font-semibold text-slate-900 dark:text-yellow-50">{props.price}</span>
      </div>
    </article>
  );
}
